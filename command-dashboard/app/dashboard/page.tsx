...
export default function DashboardPage() {
  ...
  return (
    <div className={`min-h-[calc(100vh-100px)] ${isEmergency ? 'animate-pulse' : ''}`}>
      ...
      {/* VIEW SWITCHER - Horizontal Tabs */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex gap-2 bg-slate-900/50 backdrop-blur-md rounded-xl p-1.5 border border-white/10 overflow-x-auto">
          <button onClick={() => { setActiveView('MONITORING'); router.push('/dashboard?menu=monitoring'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'MONITORING' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-300 hover:bg-white/5'}`}>🖥️ Monitor</button>
          <button onClick={() => { setActiveView('ANALYTICS'); router.push('/dashboard?menu=analytics'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'ANALYTICS' ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/30' : 'text-slate-300 hover:bg-white/5'}`}>📊 Analisis</button>
          <button onClick={() => { setActiveView('ARCHIVE'); router.push('/dashboard?menu=archive'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'ARCHIVE' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/30' : 'text-slate-300 hover:bg-white/5'}`}>🗂️ Arsip</button>
          <button onClick={() => { setActiveView('COMMUNICATION'); router.push('/dashboard?menu=communication'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'COMMUNICATION' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/30' : 'text-slate-300 hover:bg-white/5'}`}>📻 Radio</button>
          <button onClick={() => { setActiveView('TRAIN_COMMS'); router.push('/dashboard?menu=train'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'TRAIN_COMMS' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-600/30' : 'text-slate-300 hover:bg-white/5'}`}>🚆 Masinis</button>
          <button onClick={() => { setActiveView('SCHEDULE'); router.push('/dashboard?menu=schedule'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'SCHEDULE' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-300 hover:bg-white/5'}`}>📅 Jadwal</button>
          <button onClick={() => { setActiveView('MAP'); router.push('/dashboard?menu=map'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'MAP' ? 'bg-teal-600 text-white shadow-lg shadow-teal-600/30' : 'text-slate-300 hover:bg-white/5'}`}>🗺️ Peta</button>
          <button onClick={() => { setActiveView('SETTINGS'); router.push('/dashboard?menu=settings'); }} className={`px-4 py-2.5 rounded-lg font-bold text-sm whitespace-nowrap transition-all duration-300 ${activeView === 'SETTINGS' ? 'bg-slate-700 text-white shadow-lg' : 'text-slate-300 hover:bg-white/5'}`}>⚙️ Pengaturan</button>
        </div>
      </div>
